...
template <typename T>
constexpr T abs(T arg) noexcept { return arg >= 0 ? arg : -arg; }

template <typename T>
constexpr T max(T arg1, T arg2) noexcept { return (arg1 > arg2) ? arg1 : arg2; }
...
union Vec<2, T> final {
    ...
    double euclideanDistance(const Vec& rhs) const noexcept { return (*this - rhs).length(); }

    constexpr T chebyshevDistance(const Vec& rhs) const noexcept { return max(abs(x - rhs.x), abs(y - rhs.y)); }

    constexpr T manhattanDistance(const Vec& rhs) const noexcept { return abs(x - rhs.x) + abs(y - rhs.y); }
};
...
union Vec<4, T> final {
    ...
    constexpr T lengthSqr() const noexcept { return x * x + y * y + z * z + t * t; }

    constexpr bool operator ==(const Vec& r) const noexcept {
        return (x == r.x) && (y == r.y) && (z == r.z) && (t == r.t);
    }

    constexpr T dot(const Vec& r) const noexcept { return x * r.x + y * r.y + z * r.z + t * r.t; }
...
union Vec final {
    ...
    constexpr Vec operator -() const noexcept { return Vec(-less, -last); }

...
void AudioSystem::play(const filesystem::path& path, Vec3f pos, Vec3f velocity) { mPimpl->play(path, pos, velocity); }

void AudioSystem::play(const filesystem::path& path) { mPimpl->play(path); }
...
void AudioSystem::setSettings(AudioSystemSettings settings) { mPimpl->setSettings(settings); }

AudioSystemSettings AudioSystem::getSettings() const { return mPimpl->getSettings(); }
...
AudioSystemSettings AudioSystemImpl::getSettings() const { return mSettings; }
...
Buffer::Buffer(const filesystem::path& path) {
    ...
    if (res == -1)throw std::runtime_error("Failed to load this file.");
    ALenum format = 0;
    ...
    free(output);
    if (alGetError() != AL_NO_ERROR)
        throw std::runtime_error("Failed to set PCM data.");
}

Buffer::Buffer(Buffer&& rhs) noexcept: mBuffer(rhs.mBuffer) { rhs.mBuffer = moved; }

ALuint Buffer::get() const { return mBuffer; }

Buffer::~Buffer() { if (mBuffer != moved)alDeleteBuffers(1, &mBuffer); }

Source::Source() {
    alGenSources(1, &mSource);
    if (alGetError() != AL_NO_ERROR)
        throw std::runtime_error("Failed to create a source.");
}

Source::Source(Source&& rhs) noexcept: mSource(rhs.mSource) { rhs.mSource = moved; }
...
void Source::setGain(float gain) { alSourcef(mSource, AL_GAIN, gain); }
...
Source::~Source() { if (mSource != moved)alDeleteSources(1, &mSource); }

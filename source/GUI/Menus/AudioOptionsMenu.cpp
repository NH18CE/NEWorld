...
namespace Menus {
    ...
    private:
        ...
        void onLoad() override {
            mTitle = GUI::Label(GetStrbyKey("NEWorld.sound.caption"), -225, 225, 20, 36, 0.5, 0.5, 0.0, 0.0);
            auto settings = getAudioSystem().getSettings();
            mBGMGain = GUI::Trackbar(GetStrbyKey("NEWorld.sound.BGMGain"), 100, 
                static_cast<int>(settings.BGMGain * 300.0f), -200, 201,60, 84, 0.5, 0.5, 0.0, 0.0);
            mEffectGain = GUI::Trackbar(GetStrbyKey("NEWorld.sound.effectGain"), 100, 
                static_cast<int>(settings.effectGain * 300.0f), -200,201, 90, 114, 0.5, 0.5, 0.0, 0.0);
            mGUIGain = GUI::Trackbar(GetStrbyKey("NEWorld.sound.GUIGain"), 100,
                static_cast<int>(settings.GUIGain * 300.0f), -200,201, 120, 144, 0.5, 0.5, 0.0, 0.0);
            mBackBtn = GUI::Button(GetStrbyKey("NEWorld.sound.back"), -250, 250, -44, -20, 0.5, 0.5, 1.0, 1.0);
        ...
        void onUpdate() override {
            ...
            mBGMGain.text = GetStrbyKey("NEWorld.sound.BGMGain") +":"+std::to_string(mBGMGain.barpos/3)+"%";
            mEffectGain.text = GetStrbyKey("NEWorld.sound.effectGain") +":"+ std::to_string(mEffectGain.barpos / 3) + "%";
            mGUIGain.text = GetStrbyKey("NEWorld.sound.GUIGain") +":"+ std::to_string(mGUIGain.barpos / 3) + "%";
